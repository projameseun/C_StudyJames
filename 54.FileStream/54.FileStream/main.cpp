...
int main()
{
	...
	*/
	...
	//즉, 출력이라는 단어는 단순히 화면에 보여주는 것뿐 아니라, 데이터를 외부의 "전송"한다는 의미로 확장된다.

	//쓰기 -> 데이터를 저장하는 작업으로 직접적으로 의미한다.
	//하지만 C에서 "쓰기"는 단순히 파일에 데이터를 저장하는 것뿐만 아니라, 출력 스트림을 통해 데이터를 전송하는 개념과도 같다.

	//fputs,fpintf는 데이터를 "출력 스트림으로 보낸다" 
	//스트림이 콘솔이면 화면에 "출력" 
	//스트림이 파일이면 "파일에 저장되는 것이다"

	//결론적으로 "출력"과 "쓰기" 는 개념적으로 매우 비슷하며, 둘 다 데이터를 "어디론가 보낸다" 는 뜻을 포함하고있다.
	///fprintf->서식 지정이필요한 경우
	//fputs 문자열 출력에 특화됨

	// scanf 입력함수이면서 입력스트림(파일,키보드등)에 데이터를 읽어오는 함수
	//fputs와 fprintf는 데이터를 출력 스트림으로 보내는 함수이다.
	//printf는 출력 스트림으로 데이터를 보내는 함수이다 주로 데이터를 화면에 출력하는데 사용된다.
	
	//사용 되는 맥락에 따라서 다르게 표현이 될 수있다
	//출력한다 또는 보낸다 라는 말로 사용 될 수있다.

	//결론은 ===> 콘솔 화면에 출력한다, 파일에 데이터를 보낸다, 네트워크로 데이터를 보낸다.
